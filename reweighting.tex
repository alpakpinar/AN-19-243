...
The resulting SFs are shown in Fig.~\ref{fig:theory_sf_qcd_nlo_w_dy} for DY and W production. To protect the outcome of the reweighting procedure from binning effects, the binned scale factor is interpolated using a falling exponential function:
\begin{equation}
...
\subsubsection{QCD NNLO}



\iffalse
The \Zjets~and \Wjets~processes are produced at leading-order (LO) in
exlusive $H_{T}$ bins. However, the predictions for these processes
at next-to-leading-order (NLO) in QCD and electroweak (EW) show
significant improvements to the boson \pt spectra. In this analysis,
event-by-event reweighting schema is adopted to correct the LO
monte carlo simulation, through matching the generator level
distribution of the boson \pt to the corresponding NLO prediction.

LO simulations for the \Zjets, and \Wjets~processes are corrected
using boson \pt-dependent higher order QCD correction factors. These factors
are derived using MC simulation generated with \textsc{MadGraph5\_}a\textsc{MC@NLO}
and 0, 1, or 2 additional partons at NLO in QCD. The matching is performed
through \PYTHIA8.2 parton shower using the FxFx algorithm~\cite{Frederix:2012ps}.
The post shower boson \pt distributions in such samples are then compared to the
ones obtained from the samples simulated at LO in QCD, after requiring at least
one jet with generated level $\pt > 100$~\GeV and $|\eta| < 2.5$. To be in closer
agreement with the analysis phase space, an additional requirement on the
minimum $\Delta\Phi(\rm{jets,boson}) > 0.5$ of the first 4 jets is applied. The
ratio of the two distributions are then used to correct the LO in QCD processes.
In addition, \pt-dependent higher-order EW corrections extracted from theoretical
calculations~\cite{Denner:2009gj,Denner:2011vu,Denner:2012ts,Kuhn:2005gv,Kallweit:2014xda,Kallweit:2015dum}.

Fig.~\ref{fig:nlo_corrections} shows the comparisons of the boson $\pt$ distributions
of the  \Zjets~and \Wjets~processes simulated at LO, NLO in QCD, and NLO in QCD+EW.
The ratio pad, reflects the correction used in the analysis as a function of the boson \pt.

\begin{figure}[ht!]
  \begin{center}
    \includegraphics[width=0.49\textwidth]{fig/z_kfactor.pdf}
    \includegraphics[width=0.49\textwidth]{fig/w_kfactor.pdf}
    \caption{
      Comparision of the  boson $\pt$ distributions of the \Zjets~(left) and \Wjets~(right) processes
      at LO, NLO in QCD and NLO in QCD+EW.
    }
    \label{fig:nlo_corrections}
  \end{center}
\end{figure}

In the analysis both reweighted LO V+jets samples and the direct NLO V+jets (in QCD) samples are used and
compared to each other. In the main part of the text, the results are presented using the amc@NLO samples
for the V+jets processes.
\fi
